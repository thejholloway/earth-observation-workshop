---
title: "Workshop: Working with raster data and preparing satellite data for analyses"
...
Plot the first layer of the RasterBrick to see the image. T
...
In this section we will work through this process of going from a satellite image in a .img format, to a raster, to a data frame, in R. 
...
Read injune.img file for 16 June 2000 - use the stack function to import multiple layers of the image.

```{r }
injune2000 <-stack ('data/l7tmre_p093r078_20000716_dbgm5.img')
```
...
injune2000
```
Check the features of the Raster Stack object. There are 6 spectral bands in Landsat images, so there should be 6 layers. 
The resolution is 30 by 30. This is correct because Landsat images are at 30 by 30m resolution.
Later in my analysis I will need the Coordinate Reference System (CRS) to extract spatial points from my image. This is produced as 'coord.ref. in the table above.
...
```{r }
plotRGB(injune2000, r = 3, g = 2, b = 1, axes = TRUE, stretch = "lin", main = "Landsat True Colour Composite")
```
We want to extract data from this image to run statistical analyses on a dataset in dataframe format.
Create a data frame from the injune2000 Raster Stack - it includes the band values from Landsat bands 1-6.

```{r }
injune.df <-as.data.frame (injune2000) #'including the NA values there are the same number of obs for lonlat.df and injune.df.test
```
Check structure of injune.df. It is a data frame. 
...
```{r }
colnames(injune.df) <-c("Band1", "Band2","Band3", "Band4", "Band5","Band6")
```
...
Previously we calculated NVDI. Let's calculate NDVI for this Injune image and add this to the injune.df dataframe.
Note: Calculating NDVI for the whole landsat image will take some time to run. 
```{r }
NDVI <- calc(x=injune2000, fun=ndvCalc)
ndvi.df = as.data.frame(NDVI, na.rm = TRUE)
colnames(ndvi.df) <- "NDVI"
injune.df1 <- cbind(injune.df, ndvi.df[,1, drop=FALSE])
```
Produce a summary of the data frame, injune.df1, to check all the variables are there and look at the range of values.  
```{r }
summary (injune.df1)
```
