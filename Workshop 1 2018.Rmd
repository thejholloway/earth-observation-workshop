...
```
We can see there are multiple ways to calculate NDVI. 
...
names(tahiti)
```
If you want to change the name of a layer, you can adapt this code. 
```{r }
names(tahiti) <-c('Band1', 'Band2','Band3','Band4','Band5','Band6', 'cfmask')
```
...
fmask <- tahiti[[7]]
```
Remove fmask layer from the Landsat stack so there are now only 6 layers and call it 'tahiti 6'.
